...
struct AudioStream_TAG { uint32_t SoundPackType; uint32_t padding; uint32_t Offset; };
...
namespace StageBuilder
{
	...
	struct File {
		...
		uint32_t SoundPackType = 0;
		uint32_t SoundPackIndex = 0;
	};
	struct Section {
	...
	bool Build(bool Is_Auto, bool ShouldEncryptHeader);
}
