...
namespace StageDAT
{
	...
	{
#if _DEBUG
		AudioManager::JSON_Stages_Container JS_Stages_Container{};
#endif
		for (int StageIndex = 0; StageIndex < m_pStageDAT_Header->StageCount; StageIndex++)
		{
			...
			for (int TagIndex = 0; TagIndex < TagNum; TagIndex++, pTagIterator++)
			{
				if (IS_SECTION_TAG(pTagIterator->Identifier))
				{
					...
					if (pTagIterator->Identifier == DATACONFIG_SECTION_SOUND)
					{
						...
						{
#if _DEBUG
							AudioManager::JSON_SoundPack_Metadata SoundPack_Metadata{};
							SoundPack_Metadata.ID = std::format("0x{:x}", RelativeOffset >> 11);
							JS_Stages_Container.SoundPacks[StageName].push_back(SoundPack_Metadata);
#endif

							AudioStream_TAG* AudioStream  = (AudioStream_TAG*)pTagIterator;
							DataCnf_Output << std::format("SoundPack{:d}_{:d}.sdx",StreamIndex,AudioStream->SoundPackType) << std::endl;
							Utility::LogMessage(std::format("{:<30} {:08X} {:08X} {:08X} RelativeOffset:{:08X} (Mars -> us{:08x}.bnk) Offset[{:d}]",
								std::format("SoundPackType({:d})",AudioStream->SoundPackType),
								AudioStream->SoundPackType,
								AudioStream->padding,
								AudioStream->Offset,
								RelativeOffset,
								RelativeOffset >> 11,
							...
							std::filesystem::path OutPath = std::format(".\\stage\\{}\\.sound\\SoundPack{:d}_{:d}.sdx", m_pStageDAT_Toc[StageIndex].name,StreamIndex,AudioStream->SoundPackType);
...
#if _DEBUG
		std::ofstream OutJSON("DefaultOffsets.json", std::ios::binary);
		json j = JS_Stages_Container;
		OutJSON << j;
#endif
	}
...
namespace StageBuilder
{
			...
			for (auto& DirEntry : fs::recursive_directory_iterator(".\\stage"))
			{
				...
				if (Path.filename() == "data.cnf")
				{
					...
					while (std::getline(DataCnf, Line))
					{
						...
						if(ExtName == "sdx")
						{
							uint32_t SoundPackIndex;
							uint32_t SoundPackType;
							if (sscanf(FileName.c_str(), "SoundPack%d_%d.sdx",&SoundPackIndex,&SoundPackType))
							{
								FileInfo.SoundPackIndex = SoundPackIndex;
								FileInfo.SoundPackType    = SoundPackType;
							}
						}
	...
	bool Build(bool Is_UsingDataCnf, bool ShouldEncryptHeader)
	{
		std::vector<StageBuilder::StageInfo> StageInfo;
		Is_UsingDataCnf ? StageInfo = SetupPaths_LocalDataConfig() : StageInfo = SetupPathsFromDir();
		if (StageInfo.size() > 0)
		{
				...
				*(int*)WorkingPtr            = TagSize / sizeof(SectionTag);
				...
				for (int SectionIndex = 0; SectionIndex < StageInfo[StageIndex].SectionList.size(); SectionIndex++)
				{
					...
					for (auto& GameFile : CurrentSection.FileList)
					{
						...
						{
							AudioStream_TAG Tag(GameFile.SoundPackType, 0, FileOffset);
							memcpy(WorkingPtr, &Tag, sizeof(AudioStream_TAG));

							// Audio for MARS version
							size_t SoundPack_SectorOffset = (((std::uintptr_t)SectionData + FileOffset) - (std::uintptr_t)Cache) >> 11;
							AudioManager::JSON_SoundPack_Metadata JSON_NewSoundPack{};
							JSON_NewSoundPack.ID = std::format("0x{:x}",SoundPack_SectorOffset);
							JS_Stages_Container.SoundPacks[StageName].push_back(JSON_NewSoundPack);
							Utility::LogMessage(std::format("SoundPack{:d}.sdx (Type:{:d}) -> Sector ID (Mars):{:08x}",SoundPackIndex,GameFile.SoundPackType,SoundPack_SectorOffset));
			...
			// For the full version due to Denuvo protected function.
			if (ShouldEncryptHeader)
			{
				StageDAT::m_CryptoSeed = pHeader->Seed;
				StageDAT::m_cryptoKey = StageDAT::m_CryptoSeed ^ 0xF0F0;
				StageDAT::DAT_DecryptBuffer(reinterpret_cast<int*>(pHeader) + 1, 3);
			}
			...
			std::ofstream OutJSON(AudioManager::New_SoundPacks_FileName,std::ios::binary);
			json j = JS_Stages_Container;
			OutJSON << j;
			return true;
		}
