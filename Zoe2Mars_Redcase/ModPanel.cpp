...
namespace ModPanel
{
			...
			for (int LevelIndex = 0; LevelIndex < ModPanel::SpecialLevels.size(); LevelIndex++)
			{
				...
				for (int row = 0; row < 1; row++)
				{
					...
					for (int column = 0; column < 3; column++)
					{
						...
						switch (column)
						{
							case 0:ImGui::Text(ModPanel::SpecialLevels[LevelIndex]->StageName.c_str());
								break;
							case 1:ImGui::Text(std::to_string(ModPanel::SpecialLevels[LevelIndex]->g_nStoryFlag).c_str());
								break;
							case 2:
							{
								if (ImGui::Selectable(ModPanel::SpecialLevels[LevelIndex]->LevelDescription.c_str()))
								{
									strcpy_s(GameOffsets::g_CurrentStage, 16, ModPanel::SpecialLevels[LevelIndex]->StageName.c_str());
									*GameOffsets::g_nStoryFlag = ModPanel::SpecialLevels[LevelIndex]->g_nStoryFlag;
									strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res01");
									Send_LoadRequest();
								}
							}
						}
	...
	void DrawLevelSelection_AllStoryLevels()
	{
			...
			for (int LevelIndex = 0; LevelIndex < ModPanel::AllStoryLevels.size(); LevelIndex++)
			{
				...
				for (int row = 0; row < 1; row++)
				{
					...
					for (int column = 0; column < 3; column++)
					{
						...
						switch (column)
						{
							case 0:ImGui::Text(ModPanel::AllStoryLevels[LevelIndex]->StageName.c_str());
								break;
							case 1:ImGui::Text(std::to_string(ModPanel::AllStoryLevels[LevelIndex]->g_nStoryFlag).c_str());
								break;
							case 2:
							{
								if (ImGui::Selectable(ModPanel::AllStoryLevels[LevelIndex]->LevelDescription.c_str()))
								{
									if (ModPanel::AllStoryLevels[LevelIndex]->g_nStoryFlag >= 236 && ModPanel::AllStoryLevels[LevelIndex]->g_nStoryFlag <= 240)
									{
										if (*GameOffsets::Jehuty::g_pCharacterFlags == GameTypes::Jehuty::JEHUTY_DAMAGED)
										{
											strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res02");
										}
										else
										{
											strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res03");
										}
									}
									...
									strcpy_s(GameOffsets::g_CurrentStage, 16, ModPanel::AllStoryLevels[LevelIndex]->StageName.c_str());
									*GameOffsets::g_nStoryFlag = ModPanel::AllStoryLevels[LevelIndex]->g_nStoryFlag;
									Send_LoadRequest();
								}
							}
						}
	...
	void DrawMainTab()
	{
		if (ImGui::BeginTabItem("Main"))
		{
			...
			ImGui::Text("Current EXP : %d/4950000", *GameOffsets::Jehuty::g_CurrentExp);
			ImGui::SameLine();
			if (ImGui::SmallButton("Max"))
			{
				*GameOffsets::Jehuty::g_CurrentExp = 4950000;
			}
			ImGui::SameLine();
			if (ImGui::SmallButton("Give all subweapons"))
			{
				GameOffsets::Jehuty::pSubWeapons->Is_Geyser_Owned        = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Comet_Owned         = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Gauntlet_Owned      = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Decoy_Owned         = true;
				GameOffsets::Jehuty::pSubWeapons->Is_VectorCannon_Owned  = true;
				GameOffsets::Jehuty::pSubWeapons->Is_HomingMissile_Owned = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Wisp_Owned          = true;
				GameOffsets::Jehuty::pSubWeapons->Is_FloatingMine_Owned  = true;
				GameOffsets::Jehuty::pSubWeapons->Is_ZeroShift_Owned     = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Phalanx_Owned       = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Halberd_Owned       = true;
				GameOffsets::Jehuty::pSubWeapons->Is_Mummy_Owned         = true;
			}
			ImGui::Text("Jehuty Flags : %d", *GameOffsets::Jehuty::g_pCharacterFlags);
...
Do not restart/change the stage during DEMO/FMV.)");
			// Change Story Character
			{
				...
				const char* CharacterFlags_ItemLabel[] = { "Jehuty","Jehuty V2","Damaged Jehuty","Naked Jehuty" };
				...
				ImGui::SameLine();
				if (ImGui::Button("Restart and play as"))
				{
					switch (SelectedIndex)
					{
					case 0:
					{
						if (*GameOffsets::g_nStoryFlag >= 236 && *GameOffsets::g_nStoryFlag <= 267)
						{
							break;
						}
						*GameOffsets::Jehuty::g_pCharacterFlags = GameTypes::Jehuty::E_kCharacterFlags::JEHUTY;
						strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res01");
						break;
					}
					case 1:
					{
						...
						*GameOffsets::Jehuty::g_pCharacterFlags = GameTypes::Jehuty::E_kCharacterFlags::JEHUTY_V2;
						strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res01");
						break;
					}
					case 2:
					{
						*GameOffsets::Jehuty::g_pCharacterFlags = GameTypes::Jehuty::E_kCharacterFlags::JEHUTY_DAMAGED;
						strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res02");
						break;
					}
					case 3:
					{
						*GameOffsets::Jehuty::g_pCharacterFlags = GameTypes::Jehuty::E_kCharacterFlags::JEHUTY_NAKED;
						strcpy_s(GameOffsets::g_MainResident_ResourceName, 16, "res03");
						break;
					}
					}
			...
			{
				ImGui::Text("Stage : %s", GameOffsets::g_CurrentStage);
				ImGui::Text("StoryFlag : %d", *GameOffsets::g_nStoryFlag);
				ImGui::Text("Primary Resident : %s", GameOffsets::g_MainResident_ResourceName);
				ImGui::Text("Load Request : %d", *GameOffsets::g_nLoadRequest);
				ImGui::SameLine();
				if (ImGui::SmallButton("Restart Stage"))
				{
					Send_LoadRequest();
				}
				Utility::CreateTooltip_OnSameLine("Restart the stage + Destroy non resident.\n\nDo not restart/change the stage during DEMO/FMV.");
			}
			ImGui::Dummy(ImVec2(0, TextSeparator_Y_Spacing));
			ImGui::SeparatorText("All Story Levels / Specials");
			ImGui::Text("Click on the stage description to load it.");
			DrawLevelSelection();
			ImGui::EndTabItem();
		}
	}
...
the game will instead look for the files in the game directory.)");
	...
	{
		if (!*GameOffsets::g_nLoadRequest)
		{
			*GameOffsets::g_nLoadRequest = 4113;
		}
	}
